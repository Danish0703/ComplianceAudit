...
const AppCalls = ({ openModal, setModalState }: AppCallsInterface) => {
  ...
  const configuredAppId = Number(import.meta.env.VITE_COUNTER_APP_ID || 0)
  const [appId, setAppId] = useState<number | null>(configuredAppId > 0 ? configuredAppId : null)
  ...
  // Only fetch count when modal is open and an app id is configured.
  useEffect(() => {
    const load = async () => {
      if (openModal && appId) {
  ...
  }, [openModal, appId, TransactionSigner])
        ...
        <div className="flex flex-col gap-4">
          ...
          {!appId && (
            <div className="alert alert-warning">
              <span>
                No Counter App ID configured for this network. Set VITE_COUNTER_APP_ID in frontend .env to enable count fetch and increment.
              </span>
            </div>
          )}
