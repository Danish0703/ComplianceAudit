...
const Bank = ({ openModal, closeModal }: BankProps) => {
  ...
  const configuredBankAppId = Number(import.meta.env.VITE_BANK_APP_ID || 0)
  const [appId, setAppId] = useState<number | ''>(configuredBankAppId > 0 ? configuredBankAppId : 0)
  ...
  return (
    <dialog id="bank_modal" className={`modal ${openModal ? 'modal-open' : ''} bg-slate-200`}>
      <form method="dialog" className="modal-box max-w-3xl">
        ...
        {configuredBankAppId <= 0 && (
          <div className="mt-2 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-800">
            No Bank App ID configured for this network. Set VITE_BANK_APP_ID in frontend .env.
          </div>
        )}
              ...
              <ul className="text-sm">
                {statements.map((s) => (
                  <li key={s.id} className="py-1 flex justify-between items-center border-b last:border-0">
                    ...
                    <a
                      href={`https://lora.algokit.io/${explorerNetwork}/transaction/${s.id}`}
                      target="_blank"
